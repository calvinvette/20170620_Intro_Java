...
public class PaymentEndpoint {
	...
	public Response create(Payment entity) {
		...
		return Response.created(
				UriBuilder.fromResource(PaymentEndpoint.class)
						.path(String.valueOf(entity.getPaymentId())).build())
				.build();
	}
	...
	public Response findById(@PathParam("id") Long id) {
		TypedQuery<Payment> findByIdQuery = em
				.createQuery(
						"SELECT DISTINCT p FROM Payment p WHERE p.paymentId = :entityId ORDER BY p.paymentId",
						Payment.class);
	...
	@GET
	@Produces("application/json")
	public List<Payment> listAll(@QueryParam("start") Integer startPosition,
